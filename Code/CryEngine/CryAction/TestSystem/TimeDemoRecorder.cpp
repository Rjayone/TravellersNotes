...
bool CTimeDemoRecorder::RecordFrame()
{
	...
	{
		if(IHMDDevice* pHMDDevice = pHmdManager->GetHMDDevice())
		{
			const HMDTrackingState& trackingState = pHMDDevice->GetWorldTrackingState();

			if (trackingState.CheckStatusFlags(HS_IsUsable))
			{
