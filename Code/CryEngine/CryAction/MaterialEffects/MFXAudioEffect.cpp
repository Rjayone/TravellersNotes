...
void CMFXAudioEffect::Execute( const SMFXRunTimeEffectParams& params )
{
	...
	if (pOwnerEntity)
	{
		...
		pIEntityAudioProxy->ExecuteTrigger(m_audioParams.trigger.GetTriggerId(), eLSM_None, params.audioProxyId);
	}
	else
	{
		...
		if (pIAudioProxy != NULL)
		{
			...
			pIAudioProxy->SetObstructionCalcType(eAOOCT_SingleRay);
