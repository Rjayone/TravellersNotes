...
EAudioRequestStatus CAudioSystemImpl_sdlmixer::PlayFile(IATLAudioObjectData* const pAudioObjectData, char const* const szFile)
{
	return eARS_SUCCESS;
}

EAudioRequestStatus CAudioSystemImpl_sdlmixer::StopFile(IATLAudioObjectData* const pAudioObjectData, char const* const szFile)
{
	return eARS_SUCCESS;
}
...
EAudioRequestStatus CAudioSystemImpl_sdlmixer::ParseAudioFileEntry(XmlNodeRef const pAudioFileEntryNode, SATLAudioFileEntryInfo* const pFileEntryInfo)
{
	...
	if ((_stricmp(pAudioFileEntryNode->getTag(), ms_sSDLFileTag) == 0) && (pFileEntryInfo != nullptr))
	{
