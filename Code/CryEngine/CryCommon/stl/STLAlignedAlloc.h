...
public:
	...
	// This is needed by VS2015.
	// If it's not available, variadic construct with no parameters or mismatching type will (attempt to) create copies, which depending on T is potentially impossible.
	// Afterwards, the constructed objects are copied into the target memory (ie, there is a "converting construct" followed by "copy construct"/"move construct")
