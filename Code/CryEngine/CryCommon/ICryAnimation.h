...
#ifdef EDITOR_PCDEBUGCODE
	virtual uint32 GetResetMode() const = 0;  //obsolete when we remove CharEdit
	virtual void SetResetMode(uint32 rm) = 0; //obsolete when we remove CharEdit
...
struct IAnimationSet
{
	...
	virtual bool GetMotionParameters(uint32 nAnimationId, int32 parameterIndex, IDefaultSkeleton* pDefaultSkeleton, Vec4& outParameters) const = 0; // Queries motion parameters of specific animation examples. Used for blendspace editing.
