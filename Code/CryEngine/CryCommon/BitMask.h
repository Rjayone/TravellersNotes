...
struct bitmaskPtr {
	...
	bitmaskPtr& setptr(uint *newData)
	{
		...
		data=newData; refCounted=0; return *this;
	}
};
...
template<class Data,int MaxSize> struct bitmask_t {
	...
	{
		bitmask_t<bitmaskBuf<MaxSize>,MaxSize> res;	res.data.setsize(MaxSize);
		int i; for(i=0;i<data.getsize();i++) res.data[i] = ~data[i];
...
{ 
	...
	else if (nbits<33) dst.setsize(1)[0] = (uint)mask;
...
template<class Data1,class Data2,int MaxSize> ILINE bitmask_t<bitmaskBuf<MaxSize>,MaxSize> operator&(const bitmask_t<Data1,MaxSize>& op1, const bitmask_t<Data2,MaxSize>& op2)
{
	...
	for(int i=0;i<res.data.getsize();i++) res.data[i] = op1.data[i] & op2.data[i];
	return res;
}
...
template<class Data1,class Data2,int MaxSize> ILINE bitmask_t<bitmaskBuf<MaxSize>,MaxSize> operator^(const bitmask_t<Data1,MaxSize>& op1, const bitmask_t<Data2,MaxSize>& op2)
{
	...
	if (op1.data.getsize() > op2.data.getsize())
		for(i=op1.data.getsize()-1; i>=op2.data.getsize(); i--) res.data[i] = op1.data[i];
	else
		for(i=op2.data.getsize()-1; i>=op1.data.getsize(); i--) res.data[i] = op2.data[i];
	for(;i>=0;i--) res.data[i] = op1.data[i] ^ op2.data[i];
	return res;
}
...
{
	int i; for(i=op2.data.getsize()-1;i>op1.data.getsize();i--) if (op2.data[i]) return true;
	for(;i>=0;i--) 
		if (op1.data[i]<op2.data[i]) return true;
		else if (op1.data[i]>op2.data[i]) return false;
	return false;
}
...
template<class Data,int MaxSize> ILINE bitmask_t<bitmaskBuf<MaxSize>,MaxSize> operator<<(const bitmask_t<Data,MaxSize>& op, int shift)
{
	...
	while(size>0 && !op.data[size-1]) size--;
...
{
	bitmask_t<bitmaskOneBit,MaxSize> res; res.data.index=op.data.index+shift;
	return res;
}
...
template<class Data,int MaxSize> ILINE bitmask_t<bitmaskBuf<MaxSize>,MaxSize> operator-(const bitmask_t<Data,MaxSize>& op, int sub)
{
	...
	int i; for(i=0;i<res.data.getsize();i++)	
	{
		uint64 j = (uint64)res.data[i]-sub;
		res.data[i] = (uint)j; sub = -(int)(j>>32);
	}
	if (sub) for(res.data.setsize(MaxSize);i<MaxSize;i++)
		res.data[i] = ~0u;
	return res;
}
...
{
	int i; for(i=mask.data.getsize()-1;i>0 && !mask.data[i];i--);
...
template<int MaxSize> ILINE char *_ui64toa(bitmask_t<bitmaskPtr,MaxSize> mask, char *str, int radix)	// assumes radix 16
{
