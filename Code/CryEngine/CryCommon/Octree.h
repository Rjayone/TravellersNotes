...
$Id$
$DateTime$
Description: Octree for storing arbitrary primitives.
 Responsibility as to when and how to split nodes is delegated to
 the primitive container.

-------------------------------------------------------------------------
History:
- 14/04/2015   Created by Sebastien Laurent
...
public:
	...
	// Note : SplitDomains will output collapsed AABBs for some octants when the reference is outside of the inDomain.
	...
	template< typename T >
	void PushInPlace( const T& primitive, bool allowSplitting = true );
	...
	// The ones receiving a Push call will be eligible for splitting if allowed through allowSplitting.
	template< typename T >
	void Move( const T& primitiveFrom, const T& primitiveTo, bool allowSplitting = true );
...
struct COctree<TPrimitiveContainer, TUserData>::Node
{
	...
	inline bool IsCollapsed() const                                                              { return domain.min.x > domain.max.x; }
...
void COctree<TPrimitiveContainer, TUserData>::FindIntersectingLeaves( TLeaves& outNodes1, TLeaves& outNodes2, TLeaves& outNodesBoth, const T& primitive1, const T& primitive2 )
{
	if( m_root.IsLeaf() )
	{
		...
		if( intersects1 || intersects2 )
		{
			if( !intersects1 )
			{
				outNodes2.push_back( SLeafInfo( &m_root, 0 ) );
			}
			else if( !intersects2 )
			{
				outNodes1.push_back( SLeafInfo( &m_root, 0 ) );
			}
			else
			{
				outNodesBoth.push_back( SLeafInfo( &m_root, 0 ) );
			}
		}
	}
	else
...
void COctree<TPrimitiveContainer, TUserData>::FindIntersectingLeaves( TConstLeaves& outNodes, const T& primitive ) const
{
	if( m_root.IsLeaf() )
	{
		...
		if( m_root.primitiveContainer.Intersects( m_root.domain, primitive, m_pUserData ) )
		{
			outNodes.push_back( SConstLeafInfo( &m_root, 0 ) );
		}
	}
	else
	{
...
template< typename TPrimitiveContainer, typename TUserData >
template< typename T >
void COctree<TPrimitiveContainer, TUserData>::PushInPlace( const T& primitive, bool allowSplitting )
{
	TLeaves leaves;
...
template< typename TPrimitiveContainer, typename TUserData >
template< typename T >
void COctree<TPrimitiveContainer, TUserData>::Move( const T& primitiveFrom, const T& primitiveTo, bool allowSplitting )
{
	TLeaves leavesFrom;
	TLeaves leavesTo;
...
{
	CRY_ASSERT_MESSAGE( m_root.IsLeaf(), "Octree has already been split into leaves. Push() shouldn't be called anymore on it" );
	if( m_root.IsLeaf() )
	{
...
void COctree<TPrimitiveContainer, TUserData>::FindIntersectingLeavesInChunk( TLeaves& outNodes1, TLeaves& outNodes2, TLeaves& outNodesBoth, const T& primitive1, const T& primitive2, Node* pNodeChunk, size_t depth )
{
	...
	for( int i = 0; i < OctreeUtils::eOctant_Count; ++i )
	{
		...
		{
			const bool primitive1Intersects = pChildNode->primitiveContainer.Intersects( pChildNode->domain, primitive1, m_pUserData );
			const bool primitive2Intersects = pChildNode->primitiveContainer.Intersects( pChildNode->domain, primitive2, m_pUserData );
			if( primitive1Intersects || primitive2Intersects )
			{
				if( pChildNode->IsLeaf() )
				{
					SLeafInfo newLeaf( pChildNode, depth );
					if( !primitive1Intersects )
					{
						outNodes2.push_back( newLeaf );
					}
					else if( !primitive2Intersects )
					{
...
void COctree<TPrimitiveContainer, TUserData>::SplitDomains( AABB (&outDomains)[ OctreeUtils::eOctant_Count ], const AABB& inDomain, const Vec3& reference )
{
	...
	const bool xMinOk = inDomain.min.x <= reference.x;
	const bool xMaxOk = inDomain.max.x >= reference.x;
	const bool yMinOk = inDomain.min.y <= reference.y;
	const bool yMaxOk = inDomain.max.y >= reference.y;
	const bool zMinOk = inDomain.min.z <= reference.z;
	const bool zMaxOk = inDomain.max.z >= reference.z;

	const bool xIn = xMinOk && xMaxOk;
	const bool yIn = yMinOk && yMaxOk;
	const bool zIn = zMinOk && zMaxOk;
	...
	else
	{
		...
		if( xMinOk && yMinOk && zMinOk )
		{
			outDomains[ OctreeUtils::eOctant_XmYmZm ].max.x = xMaxOk ? reference.x : inDomain.max.x;
			outDomains[ OctreeUtils::eOctant_XmYmZm ].max.y = yMaxOk ? reference.y : inDomain.max.y;
			outDomains[ OctreeUtils::eOctant_XmYmZm ].max.z = zMaxOk ? reference.z : inDomain.max.z;
		}
		else
		{
		...
		if( xMaxOk && yMinOk && zMinOk )
		{
			outDomains[ OctreeUtils::eOctant_XpYmZm ].min.x = xMinOk ? reference.x : inDomain.min.x;
			outDomains[ OctreeUtils::eOctant_XpYmZm ].max.y = yMaxOk ? reference.y : inDomain.max.y;
			outDomains[ OctreeUtils::eOctant_XpYmZm ].max.z = zMaxOk ? reference.z : inDomain.max.z;
		}
		else
		{
		...
		if( xMinOk && yMaxOk && zMinOk )
		{
			outDomains[ OctreeUtils::eOctant_XmYpZm ].max.x = xMaxOk ? reference.x : inDomain.max.x;
			outDomains[ OctreeUtils::eOctant_XmYpZm ].min.y = yMinOk ? reference.y : inDomain.min.y;
			outDomains[ OctreeUtils::eOctant_XmYpZm ].max.z = zMaxOk ? reference.z : inDomain.max.z;
		}
		else
		{
		...
		if( xMaxOk && yMaxOk && zMinOk )
		{
			outDomains[ OctreeUtils::eOctant_XpYpZm ].min.x = xMinOk ? reference.x : inDomain.min.x;
			outDomains[ OctreeUtils::eOctant_XpYpZm ].min.y = yMinOk ? reference.y : inDomain.min.y;
			outDomains[ OctreeUtils::eOctant_XpYpZm ].max.z = zMaxOk ? reference.z : inDomain.max.z;
		}
		else
		{
		...
		if( xMinOk && yMinOk && zMaxOk )
		{
			outDomains[ OctreeUtils::eOctant_XmYmZp ].max.x = xMaxOk ? reference.x : inDomain.max.x;
			outDomains[ OctreeUtils::eOctant_XmYmZp ].max.y = yMaxOk ? reference.y : inDomain.max.y;
			outDomains[ OctreeUtils::eOctant_XmYmZp ].min.z = zMinOk ? reference.z : inDomain.min.z;
		}
		else
		{
		...
		if( xMaxOk && yMinOk && zMaxOk )
		{
			outDomains[ OctreeUtils::eOctant_XpYmZp ].min.x = xMinOk ? reference.x : inDomain.min.x;
			outDomains[ OctreeUtils::eOctant_XpYmZp ].max.y = yMaxOk ? reference.y : inDomain.max.y;
			outDomains[ OctreeUtils::eOctant_XpYmZp ].min.z = zMinOk ? reference.z : inDomain.min.z;
		}
		else
		{
		...
		if( xMinOk && yMaxOk && zMaxOk )
		{
			outDomains[ OctreeUtils::eOctant_XmYpZp ].max.x = xMaxOk ? reference.x : inDomain.max.x;
			outDomains[ OctreeUtils::eOctant_XmYpZp ].min.y = yMinOk ? reference.y : inDomain.min.y;
			outDomains[ OctreeUtils::eOctant_XmYpZp ].min.z = zMinOk ? reference.z : inDomain.min.z;
		}
		else
		{
		...
		if( xMaxOk && yMaxOk && zMaxOk )
		{
			outDomains[ OctreeUtils::eOctant_XpYpZp ].min.x = xMinOk ? reference.x : inDomain.min.x;
			outDomains[ OctreeUtils::eOctant_XpYpZp ].min.y = yMinOk ? reference.y : inDomain.min.y;
			outDomains[ OctreeUtils::eOctant_XpYpZp ].min.z = zMinOk ? reference.z : inDomain.min.z;
		}
		else
		{
