...
#if (CRY_PLATFORM_LINUX || CRY_PLATFORM_ANDROID || CRY_PLATFORM_APPLE || CRY_PLATFORM_ORBIS) && (!defined(_RELEASE) || defined(_DEBUG))
	struct SAssertData
	{
		int line;
...
#if CRY_PLATFORM_ORBIS
	char buf[512];
	const char *path = ConvertFileName(buf, sizeof(buf), dirname);
...
/*	LONG r;
	__asm__ __volatile__ (
	#if (CRY_PLATFORM_LINUX && CRY_PLATFORM_64BIT) || CRY_PLATFORM_APPLE  // long is 64 bits on amd64.
		"lock ; xaddq %0, (%1) \n\t"
	#else
		"lock ; xaddl %0, (%1) \n\t"
	#endif
...
#if (CRY_PLATFORM_LINUX && CRY_PLATFORM_64BIT) || CRY_PLATFORM_MAC || defined(IOS_SIMULATOR)
...
int WrappedStat(const char * _Filename, struct stat * _Stat)
{
	...
	char buf[512];
#if CRY_PLATFORM_ORBIS
	return ::stat(ConvertFileName(buf, sizeof(buf), _Filename), _Stat);
#else
