...
{
	ePEF_Independent = BIT(0),			// Deprecated: Emitter auto-serializes itself. Now occurs automatically when no external references.
...
struct SpawnParams
{
	...
	EAudioObjectObstructionCalcType	obstOcclCalcType; // Audio obstruction/occlusion calculation type.
	string				sAudioRTPC;				// Indicates what audio RTPC this particle effect instance drives.
	...
	inline SpawnParams( EGeomType eType = GeomType_None, EGeomForm eForm = GeomForm_Surface )
	{
		...
		fPulsePeriod = 0;
		fStrength = -1;
		obstOcclCalcType = eAOOCT_Ignore;
	}
};
