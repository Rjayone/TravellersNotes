...
{
	WCHAR filePath[512];
	size_t nLen = GetModuleFileNameW(GetModuleHandle(NULL), filePath, sizeof(filePath) / sizeof(filePath[0]));

	if (nLen >= sizeof(filePath) / sizeof(filePath[0]))
	{
...
#if CRY_PLATFORM_WINAPI || CRY_PLATFORM_LINUX || CRY_PLATFORM_ANDROID
//////////////////////////////////////////////////////////////////////////
void CryFindEngineRootFolder(unsigned int engineRootPathSize, char* szEngineRootPath)
{
	...
	CryGetExecutableFolder(sizeof(szExecFilePath) / sizeof(szExecFilePath[0]), szExecFilePath);
	...
	} while (nCurDirSlashPos != string::npos);

	if (nCurDirSlashPos == string::npos)
	{
...
void CryFindRootFolderAndSetAsCurrentWorkingDirectory()
{
	...
	CryFindEngineRootFolder(sizeof(szEngineRootDir) / sizeof(szEngineRootDir[0]), szEngineRootDir);
	CrySetCurrentWorkingDirectory(szEngineRootDir);
}

#endif // CRY_PLATFORM_WINAPI || CRY_PLATFORM_POSIX && !CRY_PLATFORM_ORBIS

#if CRY_PLATFORM_DURANGO && (!defined(_LIB) || defined(PERFORMANCE_BUILD))
