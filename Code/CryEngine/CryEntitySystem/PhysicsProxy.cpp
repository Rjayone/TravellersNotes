...
{ 
	int id = nSlot;
	int nLevels,nBits; ParsePartId(id, nLevels,nBits);
...
{
	int nLevels,nBits; ParsePartId(idPart, nLevels,nBits);
...
{
	int id = ilog2((usedMask ^ usedMask+1)+1)-1;
...
{
	if (pent->m_pBinds) for(uint i=0;i<pent->m_pBinds->childs.size();i++) 
	{
		int idOld = pent->m_pBinds->childs[i]->m_pBinds->attachId,  idNew = idmap[idOld] = AllocAttachId(idmaskDst);
	...
	case ENTITY_EVENT_XFORM:
		...
		if (m_pPhysicalEntity && m_pPhysicalEntity->GetType()<=PE_RIGID && m_pEntity->m_pBinds && m_pEntity->m_pBinds->attachId>=0)
		{
					...
					pAdamProxy->m_pPhysicalEntity && pAdamProxy->m_pPhysicalEntity->GetType()<=PE_WHEELEDVEHICLE)
			{
				...
				{
					int nLevels,nBits; ParsePartId(sp.partid,nLevels,nBits);
	...
	case ENTITY_EVENT_ATTACH:	
		{
			...
			if (pChild)
			{
						...
						(i=pAdamProxy->m_pPhysicalEntity->GetType())<=PE_WHEELEDVEHICLE && i!=PE_STATIC)
				{
					...
					int idmap[PARTID_MAX_ATTACHMENTS+1]; memset(idmap, -1, sizeof(idmap));
	...
	case ENTITY_EVENT_MATERIAL:
		if (IMaterial *pMtl = (IMaterial*)event.nParam[0]) 
		{
			...
			if (m_pEntity->m_pBinds && m_pEntity->m_pBinds->attachId>=0 && pAdam && pAdam!=m_pEntity && pAdam->GetPhysics())	{
				ppart.partid = GetPartId0();	pAdam->GetPhysics()->SetParams(&ppart);
