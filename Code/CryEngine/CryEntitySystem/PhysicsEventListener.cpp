...
CEntity *FindAttachId(CEntity *pent, int attachId)
{
	if (pent->GetPhysicalProxy() && pent->GetPhysicalProxy()->GetPhysAttachId()==attachId)
		return pent;
	CEntity *pMatch;
	for(int i=pent->GetChildCount()-1;i>=0;i--)	if (pMatch=FindAttachId((CEntity*)pent->GetChild(i),attachId))
		return pMatch;
	return 0;
}
...
{
	CEntity *pChild;
	int nLevels,nBits; ParsePartId(partId, nLevels,nBits);
...
int CPhysicsEventListener::OnCollision( const EventPhys *pEvent )
{
	...
	int nLevels,nBits; ParsePartId(pCollision->partid[0], nLevels,nBits);
