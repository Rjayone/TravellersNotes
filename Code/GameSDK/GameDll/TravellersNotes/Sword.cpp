...
void CSword::ProcessEvent(SEntityEvent& event)
{
	...
	{
		GetEntity()->KillTimer((int)event.nParam[0]);
...
{
	if (!IsOwnerClient() && IsBusy() || m_CombatStance->GetStance() == e_CombatStanceDefence)
		return;
...
{
	if (m_CombatStance->GetStance() == e_CombatStanceDefence)
		return;
...
{
	if (!IsOwnerClient() || !m_Holding || !m_bBusy || m_bNowAttack || m_CombatStance->GetStance() == e_CombatStanceDefence)
		return;
	...
	GetEntity()->SetTimer(_HOLDING_RESET_STATUS, (int)duration * 10);
	m_bNowAttack = true;
