...
{
	//Выделяем память под таймеры. 1 для переключения дилогов вслед за озвучкой, 2 для ответов игрока
	m_pDialogTimer.push_back(new CTimer());
	m_pDialogTimer.push_back(new CTimer());
	m_pDialogTimer[0]->AttachEventListener(this);
	m_pDialogTimer[1]->AttachEventListener(this);
	...
	m_bStartTimerForAnswering = false;
...
void CUIDialogMenu::StartDialog()
{
	...
	if (m_pNPCAnswer[0]->withoutMessage == true)
	{
		...
		//Если есть следующи диалог
		if (m_pNPCAnswer[0]->nextPhraseId != 0)
			ContinueDialog(m_pNPCAnswer[0]->nextPhraseId);
		else {
	...
	m_pDialogTimer[0]->StartTimer(m_pNPCAnswer[0]->audioDialogDuration / 100, m_pNPCAnswer[0]->nextPhraseId);
	m_pDialogTimer[0]->AttachEventListener(this);
...
void CUIDialogMenu::ContinueDialog(int nextNPCDialogId)
{
	...
	for (int i = 0; i < m_pNPCAnswer.size(); i++)
	{
		if (m_pNPCAnswer[i]->currentPhraseId == nextNPCDialogId /*&& !strcmp(m_pNPCAnswer[i]->specialEvent, "")*/)
		{
			...
			if (m_pNPCAnswer[i]->timeForAnswering > 0)
			{
				m_bStartTimerForAnswering = true;
			}
			...
			m_pDialogTimer[0]->StartTimer(m_pNPCAnswer[i]->audioDialogDuration / 100, m_pNPCAnswer[i]->nextPhraseId);
			m_pDialogTimer[0]->AttachEventListener(this);
	...
	if (nextNPCDialogId == 0)
	{
		...
		for (int i = 0; i < m_pPlayerAnswer.size(); i++)
		{
			if (m_pPlayerAnswer[i]->parentId == m_nLastParentDialogId)
			{
				...
				//Если игрок должен быстро ответить, то получаем родительский диалог и время на ответ
				if (m_bStartTimerForAnswering == true)
				{
					SDialogNPCAnswer* npcAnswer = GetParentDialogAnswer(m_nLastParentDialogId);
					if (npcAnswer != NULL)
					{
						m_pDialogTimer[1]->StartTimer(npcAnswer->timeForAnswering / 100, npcAnswer->failedToAnswerNextDialogId);
						m_pDialogTimer[1]->AttachEventListener(this);
						m_bStartTimerForAnswering = false;
					}
				} //~
...
//-------------------------------------------------------------------
SDialogNPCAnswer* CUIDialogMenu::GetParentDialogAnswer(int parentId)
{
	for (int i = 0; i < m_pNPCAnswer.size(); i++)
	{
		if (m_pNPCAnswer[i]->currentPhraseId == parentId)
			return m_pNPCAnswer[i];
	}
	return NULL;
}
...
void CUIDialogMenu::ParseXML()
{
	...
	if (DialogDescriptionFile == NULL) {
		CryLog("Failed to load dialog %s", m_pFilePath);
		return;
	}
	...
	for (int i = 0; i < rootChildCount; i++)
	{
		...
		if (!strcmp(tagName, "Phrase"))
		{
			...
			npcAnswer->audioDialogDuration = atof(pDialogItem->getAttr("duration"));
			...
			for (int j = 0; j < phraseChildCount; j++)
			{
				...
				if (!strcmp(pAnswer->getTag(), "actions"))
				{
					...
					for (int k = 0; k < actionsCount; k++)
					{
						...
						if (!strcmp(actionName, "timer"))
						{
							npcAnswer->timeForAnswering = atof(action->getAttr("timeforanswering"));
							npcAnswer->failedToAnswerNextDialogId = atoi(action->getAttr("failedToAnswerNextDialogId"));
						}
					}
				}// ~actions
				...
				if (!strcmp(pAnswer->getTag(), "PlayerAnswer"))
				{
					...
					pPlayerAnswer->audioDialogDuration = atof(pAnswer->getAttr("duration"));
