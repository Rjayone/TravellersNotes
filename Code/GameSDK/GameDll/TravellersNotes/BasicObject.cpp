...
void CBasicObject::OnPickUp(EntityId id)
{
	...
	if (pInventory->m_pItemsArray.size() > pInventory->GetMaxSlotsCount())//Если инвентарь полный
	{
		...
		action.impulse.Set(MathUtils::Random(0, 0.5), MathUtils::Random(0, 0.5), MathUtils::Random(0, 1));
		action.angImpulse.Set(MathUtils::Random(0, 0.5), MathUtils::Random(0, 0.5), MathUtils::Random(0, 0.5));
...
void CBasicObject::OnDrop(SInventoryItem* pItem, EntityId id)
{
	...
	action.angImpulse.Set(MathUtils::Random(0, 1), MathUtils::Random(0, 1), MathUtils::Random(0, 1));
