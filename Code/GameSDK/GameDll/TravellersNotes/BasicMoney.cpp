...
void CBasicMoney::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	if (m_bSparkEnable)
	{
		if (gEnv->pTimer->GetCurrTime() - m_fSparkTimer >= 10 + MathUtils::Random(0, 3) && m_pParticalEntity)
		{
			//Надо проверить на производительность
			StartSpark();
			m_fSparkTimer = gEnv->pTimer->GetAsyncCurTime() + MathUtils::Random(0, 5);
		}
	}
}
...
void CBasicMoney::OnPickUp(EntityId id)
{
	...
	if (rand)
	{
		...
		money = MathUtils::Random(MathUtils::Random(1, 15), MathUtils::Random(25, 60));
	}
...
void CBasicMoney::OnDrop(SInventoryItem *pItem, EntityId id)
{
	...
	action.angImpulse.Set((float)MathUtils::Random(0, 1), (float)MathUtils::Random(0, 1), (float)MathUtils::Random(0, 1));
