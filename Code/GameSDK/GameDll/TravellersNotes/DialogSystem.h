...
struct SDialogNPCAnswer 
{
	...
	float timeForAnswering;		//Время в течении которого игрок должен ответить на диалог
	int failedToAnswerNextDialogId; //ид диалога, если игрок не успел ответить в отведенное время
	float audioDialogDuration;  //Длинна аудиофайла в милисек
	string audioName;			//Аудио-файл выбранного диалога
	...
	SDialogNPCAnswer() : currentPhraseId(-1),
		nextPhraseId(-1),
		audioDialogDuration(0.0f),
...
public:
	...
	//Описание:
	//Возвращает указатель на фразу родительского диалога
	SDialogNPCAnswer* GetParentDialogAnswer(int parentId);
...
private:
	std::vector<CTimer*> m_pDialogTimer;
	...
	bool m_bStartTimerForAnswering; //Если игроку нужно быстро ответить на диалог
