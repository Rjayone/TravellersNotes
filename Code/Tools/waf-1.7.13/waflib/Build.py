...
class BuildContext(Context.Context):
	...
	def recurse(self, dirs, name=None, mandatory=True, once=True):
		"""Wrap the recurse method to propagate module includes automatically."""
		# To make sure all build contexts have proper information, we handle
		# this after the recurse step, but since this is a recursive method, we
		# patch in the old one for the actual recursion.
		# This ensures all BuildContexts perform the operation at the correct
		# time.
		temp = self.recurse
		self.recurse = super(BuildContext, self).recurse
		self.recurse(dirs, name, mandatory, once)
		self.recurse = temp
		self.PostprocessBuildModules()
...
Could not find a task generator for the name %r.
Please verify that your selected project is configurated to be build.
Current Settings: Spec: %r, Config: %r, Platform %r""" % (name, Options.options.project_spec, self.env['CONFIGURATION'], self.env['PLATFORM']) )
