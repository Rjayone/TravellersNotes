...
def load_msvc_common_settings(conf):
	...
	msvc_version = v['MSVC_VERSION']
	if isinstance(msvc_version, basestring) and float(msvc_version) >= 12:
		COMMON_COMPILER_FLAGS += [
			'/wd4316',	# 'T': object allocated on the heap may not be aligned X (this MUST be re-enabled at some point)
			'/wd4838',	# conversion from 'T1' to 'T2' requires a narrowing conversion
			'/wd4577',	# 'noexcept' used with no exception handling mode specified (this is OK since we don't use exceptions)
			'/wd4267',  # '=': conversion from 'T1' to 'T2', possible loss of data
			]
